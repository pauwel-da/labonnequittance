...
import { saveTrialDraft, sendTrialOtp } from './actions'
...
export default function TrialForm() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setServerError(null)
    const email = data.bailleur.email.trim()
    startTransition(async () => {
      ...
      const draftRes = await saveTrialDraft(payload)
      if ('error' in draftRes) { setServerError(draftRes.error); return }

      try { localStorage.removeItem(STORAGE_KEY) } catch {}

      if (isGmail(email)) {
        // Flux Gmail : OAuth Google avec login_hint pour pré-sélectionner le bon compte
        const supabase = createClient()
        await supabase.auth.signInWithOAuth({
          provider: 'google',
          options: {
            redirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(draftRes.next)}`,
      ...
      // Flux OTP : envoi du code par email
      const otpRes = await sendTrialOtp(email)
      if ('error' in otpRes) { setServerError(otpRes.error); return }
          ...
          <div className="flex gap-3 pt-2">
            ...
            {step < 3 ? (
              ...
              >
                ...
                ) : isGmail(data.bailleur.email) ? (
                  <><GoogleIcon /> Voir ma quittance</>
                ) : (
                  <>📄 Voir ma quittance</>
                )}
              </button>
